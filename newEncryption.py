...
def binaryToConstellation(binaryString):
    ...
    return constellation, added

def constellationToBinary(constellation, added):
...
def findVertex(constellation,b):
    ...
    currentNode = ncf.Node(constellationString[0])
...
def decrypt(key,encrypted,added):
    ...
    if len(constellationString)%2 == 0: #we avoid removing a legitimate final S
        ...
        constellationString = constellationString[0:len(constellationString) - 3] #remove stop signal
    constellationList = [constellationString[i:i + 2] for i in range(0, len(constellationString), 2)]
    rawBinary = constellationToBinary(constellationList,added)
    return ' '.join(rawBinary[i:i+7] for i in range(0, len(rawBinary), 7))
...
print('textToBinary')
binaryString = textToBinary('LikeAndSubscribe')
print(binaryString)

#step 2: Binary to Constellation
#binaryString = '00111101 00110011'
print('binaryToConstellation')
constellation, added = binaryToConstellation(binaryString)
print(constellation)

#step 2.5: constellation to galaxy
galaxy = key[0]+decoy+''.join(constellation)
...
#step 3.5: get solution for a0 of key
...
encrypted = (solution_a0G[0]*b + solution_a0G[1] - solution_a0K[1])//solution_a0K[0]
print(encrypted)
...
#step 5: decrypt
print('decrypt')
decrypted = decrypt(key,encrypted,added)
text = binaryToText(decrypted)
print(text)
...
#vertex = findVertex(galaxy,0)
#print(len(str(bin(vertex))))

#step 4: vertex to binary
#print('vertex to binary')
#newBinaryString = vertexToBinary(vertex)
